...
// used to consturct string for input, per word
temp = document.getElementById("change-make").textContent;

// the display stack implementation
...
stackspace = 7;
// used to monitor input time per word and push to the stack
let times = new Array();

nowTime = Date.now();
times.push(nowTime);

// harcoded array for 1050 words, from which a word is randomly thrown onto the screen
function giveWords() {
  l =
...
// function to fix the display bug, where the margins of the keyboard seemed to be missing
...
// Funtion to gradually increase rate of diplay of the words
var myFunction = function () {
  rate *= 0.97;
  // condition to check Game Over logic
  if (words.length < stackspace) {
    // as long as words in stack are less than 7 (end game logic) the following code is executed
...
document.addEventListener("keydown", (e) => {
  try {
    ...
    // when enter is clicked, if the words match the score is updated and the displayed keyboard correctly indicates the key being pressd

    if (e.key != "Enter") {
      if (e.key != "Backspace") {
        // this is where we update the temp string if the character at position i of the input matches the same index value of the given word
        if (e.key == words[count][letterCount]) {
          ...
          // on wrong input, change key color to red, and play Error Sound
          combo = 0;
          multiplier = 1;
    ...
    // these are the events being triggered on  Enter being pressed
    if (e.key == "Enter") {
      // as long as words match the following logic is executed. We always check words[0] as we have designed the game to pop the First word, on a matched input from the user
      if (temp == words[0]) {
        ...
        document.getElementById("change-make").textContent = "";
        words.shift();
        // beautifying the display function
        document.getElementById("words-array").textContent = words.join(" ");
        letterCount = 0;
        combo += 1;
        console.log(times);
        // logic to update the multiplier
        if (combo == 5) {
          multiplier += 1;
          multiplierText.textContent = `${multiplier}x`;
          combo = 0;
        }
        // this ensures that the times array stores times in secs and not ms.
        times.push(endTime / 1000);

        scoreTemp = Number(defaultScore);
        // logic to calculate score in a fashion that punishes delayed inputs; i.e; gives more points for quicker inputs
        // 21 secs is the max length of the game (as init rate is 3 secs and stack space is 7).
...
document.addEventListener("keyup", (e) => {
  try {
    let key = document.getElementById(`${e.code}`);
    setTimeout(() => {
      // when pressed key is released this is the following code that will be execturd.
      // the keyboards state is basically reset.
